...
void CTest_StaticExDlg::OnCbnSelchangeComboHAlign()
{
	...
	{
		case 1:
			m_text_align = DT_CENTER;
			m_static[3].set_halign(DT_CENTER);
			break;
		case 2:
			m_text_align = DT_RIGHT;
			m_static[3].set_halign(DT_RIGHT);
			break;
		default:
			m_text_align = DT_LEFT;
			m_static[3].set_halign(DT_LEFT);
			break;
	}
...
{
	int index = m_combo_valign.GetCurSel();
	if (index < 0 || index >= m_combo_valign.GetCount())
		return;

	switch (index)
	{
		case 1:
			m_line_align = DT_VCENTER;
			break;
		case 2:
			m_line_align = DT_BOTTOM;
			break;
		default:
			m_line_align = DT_TOP;
			break;
